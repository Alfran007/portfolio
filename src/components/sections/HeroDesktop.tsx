...
// `HeroDesktop` only renders on desktop (server picks the branch based on
// UA), so this dynamic chunk + the entire R3F/three/drei dependency tree
// stays out of the mobile bundle entirely.
...
/**
 * Desktop Hero. Client component because of the role rotator
 * (`useState`) and the framer-motion staggered entrance. The WebGL
 * `HeroScene` is dynamically imported with `ssr: false` — the loading
 * placeholder runs in the SSR HTML, the real Canvas mounts on the
 * client and dispatches `hero-ready` from its 3rd `useFrame`.
 */
export default function HeroDesktop() {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute inset-x-0 top-0 md:-bottom-[38vh] -bottom-[20vh] z-0 pointer-events-none select-none"
        style={{
          maskImage:
